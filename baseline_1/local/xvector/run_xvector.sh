...
stage=1 # stage is 1 means need to make egs
data=
nnet_dir=
egs_dir=
nj=40
...
# train
train_stage=-1 # Preparing the initial network
use_gpu=true
remove_egs=true
xvector_dim=100
num_jobs_initial=2
...
if [ $stage -le 5 ]; then
  ...
  relu-batchnorm-layer name=tdnn6 dim=${xvector_dim} input=stats
