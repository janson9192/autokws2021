...
def run(dataset_dir, solution_dir, code_dir):
    """run"""
    ...
    command_scoring = (
        f'python {path_scoring} --solution_dir={solution_dir}')
...
def _parse_args():
    ...
    default_dataset_dir = join(default_starting_kit_dir, 'sample_data', 'practice')
    default_solution_dir = join(default_starting_kit_dir, 'sample_data', 'practice_solution')
    default_code_dir = join(default_starting_kit_dir, 'code_submission')

    parser = argparse.ArgumentParser()
    parser.add_argument('--dataset_dir', type=str,
                        default=default_dataset_dir,
                        help="Absolute directory, directory storing the dataset, should contain"
                             "'enroll' and 'test''")

    parser.add_argument('--solution_dir', type=str,
                        default=default_solution_dir,
                        help="Absolute directory, directory storing the solutions, should contain"
                             "solution file of each dataset")
...
def main():
    ...
    logging.info("#" * 50)
    run(dataset_dir, solution_dir, code_dir)
